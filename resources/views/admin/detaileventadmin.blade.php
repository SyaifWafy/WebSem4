...
<body>
    ...
    <a href="{{ route('editEventAdmin', $event->kd_event) }}" class="btn btn-primary">Edit</a>
    <form action="{{ route('deleteEventAdmin', $event->kd_event) }}" method="POST" style="display: inline-block;">
        @csrf
        @method('DELETE')
        <button type="submit" class="btn btn-primary" onclick="return confirm('Apakah Anda yakin ingin menghapus data event ini?')">Delete</button>
    </form>
    <br>
    <br>
